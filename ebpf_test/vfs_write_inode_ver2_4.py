...
def print_who_write():
    ...
    for k, v in ret_file_data.items_lookup_and_delete_batch():
        ...
        if file_name not in not_print_list:            
            write_data =[]
            write_data = [print_type,v.pid, v.tid, v.ret_file, v.ret_file_address, v.file, v.file_address, file_name,process_name,v.i_ino,v.file_uid.val,v.file_gid.val, v.uid.val,v.euid.val,v.setuid,cur_time,'ret_file_data' ]
            print(write_data)
        #writer.writerow(write_data)
        is_print = 1
        check = 1
